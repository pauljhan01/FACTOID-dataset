...
class GatV2Classification(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_heads=4, nclass=2, num_layers=2, dropout=0.5):
        ...
        # First layer
        self.layers.append(
           GATv2Conv(in_channels, hidden_channels, heads=num_heads, concat=False)
        )

        # Middle layers
        for _ in range(self.num_layers - 2):
           self.layers.append( GATv2Conv(hidden_channels, hidden_channels, heads=num_heads, concat=False)
        )
